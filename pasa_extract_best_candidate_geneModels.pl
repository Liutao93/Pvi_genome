...
my $usage = <<USAGE;
Usage:
    perl $0 assemblies.fasta.transdecoder.genome.gff3 *.assemblies.fasta.transdecoder.cds Min_exon_number_of*s_length Min_cds_ratio_vs_cDNA Max_intron_length_ratio Max_cds_length_ratio
For Example:
    perl $0 assemblies.fasta.transdecoder.genome.gff3 *.assemblies.fasta.transdecoder.cds 3 900 0.60 0.95 0.95 > best_candidates.gff3
    1. 按照先后顺序对assemblies.fasta.transdecoder.genome.gff3中的基因模型进行过滤。
    2. 首先，根据*.assemblies.fasta.transdecoder.cds文件中的complete关键词过滤不完整基因模型。
    3. 再过滤cds数目<3的基因模型。
    4. 再过滤cds长度<900的基因模型。
