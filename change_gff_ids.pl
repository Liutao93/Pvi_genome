...
foreach my $contig_id (sort {sort_contigs($a, $b, $sort_map)} keys %ids) {
    ...
    $abrv_chr=$sort_map->{$contig_id};
    $abrv_chr= sprintf '%02s', $abrv_chr; #添加染色体前缀
    #print("xxxxxxxxxxxxxxx $abrv_chr - $contig_id --- \n");
	...
	while(! $new_id){
	    ...
	    $new_id =~ s/\@/$abrv_chr/g; #add abrevition
  ...
  $old2new{$gid}=$new_id;
  #$new2old{$new_id}=$gid;
  $old2new{$gname}=$new_id if($gname && $gname ne $gid);
 # $new2old{$new_id}=$gid if($gname && $gname ne $gid);
	...
	foreach my $mRNA (sort {$a->{start} <=> $b->{start} || $a->{end} <=> $b->{end}} values %{$gene->{mRNAs}}){
      ...
      $old2new{$tid}=$new_id;
       # $new2old{$new_id}=$tid;
       $old2new{$tname}=$new_id if($tname && $tname ne $gid);
       #$new2old{$new_id}=$tname if($tname && $tname ne $gid);
...
#######huangls 
open ( $IN, '<', $gff_file) or die "ERROR: Can't open $gff_file for reading\n$!\n";
while (<$IN>) {
    last if /^\#\#FASTA|^>/;
    next if /^\s*\#/;
	...
	my ($parent) = $attrb_text =~ /Parent=([^\;\n]*)/;	
	#print "$type\n";
  if($type ne "gene" && $type ne "mRNA"){
    my ($oldid,$su)=$id=~/^(.+)\.((exon|utr|cds).+)/;
    print "$id\t$old2new{$oldid}.$su\n";
  }
...
sub parse_sort_order {
	...
	%sort_order = map {chomp $_; split(/\s+/, $_)} <$IN>;
	close $IN;
    }
    else{
	my $flag;
...
sub sort_contigs {
	my ($a, $b, $sort_map) = @_;
	return ($sort_map->{$a} cmp $sort_map->{$b}) if defined $sort_order;
	return ($sort_map->{$a} cmp $sort_map->{$b});
}
