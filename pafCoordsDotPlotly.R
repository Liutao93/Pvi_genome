...
option_list <- list(
		...
		make_option(c("-W","--width"), type="numeric", default=7,
				help="plot size width inches [default %default]",
				dest="width"),
		make_option(c("-H","--height"), type="numeric", default=5,
				help="plot size height inches [default %default]",
				dest="height"),
		make_option(c("-l", "--show-lines"), action="store_true", default=FALSE,
				help="turn on horizontal and vertical lines on plot for separating scaffolds  [default %default]",
				dest="lines"),
		...
		make_option(c("-r", "--reference-ids"), type="character", default=NULL,
				help="comma-separated list of reference IDs to keep order [default %default]",
				dest="refIDs")
)
...
#按照REF的ID顺序排序
#alignments$refID=factor(alignments$refID,levels=c("chr01", "chr02",
#"chr03",
#"chr04",
#"chr05",
#"chr06",
...
ychr=yTickMarks-c(0,yTickMarks[2:length(yTickMarks)-1])
...
if (opt$similarity) {
	gp = ggplot(alignments) +
			...
			scale_x_continuous(breaks = cumsum(as.numeric(chromMax))-as.numeric(chromMax)/2,labels = levels(alignments$refID)) +
			theme_bw() +
			...
			scale_y_continuous(breaks =yTickMarks- ychr/2,labels = substr(levels(alignments$queryID), start = 1, stop = 20)) +
			{ if(opt$lines){ 
					geom_hline(yintercept = c(0,yTickMarks),
							color = "grey60",linetype = "longdash",
							size = .1)
				}} +
			{ if(opt$lines){ 
							geom_vline(xintercept = c(0,cumsum(as.numeric(chromMax))),
									color = "grey60",linetype = "longdash",
									size = .1) 
				}} +
...
# gp
ggsave(filename = paste0(opt$output_filename, ".png"), width = opt$width, height = opt$height, units = "in", dpi = 300, limitsize = F)
ggsave(filename = paste0(opt$output_filename, ".pdf"), width = opt$width, height = opt$height, units = "in",  limitsize = F)
